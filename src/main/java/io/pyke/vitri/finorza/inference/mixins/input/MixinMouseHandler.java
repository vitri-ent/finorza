...
import io.pyke.vitri.finorza.inference.api.IMouseHandler;
import io.pyke.vitri.finorza.inference.client.AgentControl;

@Mixin(MouseHandler.class)
public abstract class MixinMouseHandler implements IMouseHandler {
	@Shadow
	@Final
	...
	@ModifyArg(
		method = "turnPlayer()V",
		at = @At(value = "INVOKE", target = "Lnet/minecraft/util/SmoothDouble;getNewDeltaValue(DD)D"), index = 1
	)
	...
	public void vitri$onMove(double xpos, double ypos) {
		try {
			...
			if (this.minecraft.screen != null) {
				double retinaFactor = (double) minecraft.getMainRenderTarget().viewWidth / minecraft.getWindow()
					.getWidth();
	...
	@Redirect(
		method = "grabMouse()V", at = @At(
		value = "FIELD", target = "Lnet/minecraft/client/MouseHandler;ignoreFirstMove:Z", opcode = Opcodes.PUTFIELD
	)
	)
	...
	@Redirect(
		method = "onMove(JDD)V", at = @At(
		value = "FIELD", target = "Lnet/minecraft/client/MouseHandler;ignoreFirstMove:Z", opcode = Opcodes.GETFIELD
	)
	)
